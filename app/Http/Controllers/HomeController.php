...
class HomeController extends Controller
{
    ...
    public function lottery_details_ajax(Request $request)
    {
        ...
        foreach ($lotteryNumbers as $item) {
            $lotteryItems = "<input type= '$lottery_type' name='$selectedNumber' value='$item->number' class='single-number lotteryNumbers'><label>$item->number</label>";
        ...
        for ($i = 1; $i <= $lotteryWinningAmount; $i++) {
            $lotteryWinningItems = "<input type='radio' name='lotteryWinningItems' value='$i' class='winning-number'><label>X$i</label>";
