$(document).ready(function ($) {
    ...
    $(singleNum2).on("click", function () {
        var singleNumLength = $(".number-box.special").find(
            ".single-number.selected.special"
        ).length;
        var this_btn_number = parseInt($(this).text());
        ...
        if ($(this).hasClass("selected") && $(this).hasClass("special")) {
            ...
            if (singleNumLength < 1) {
                ...
                $(".result-number-palate").append(
                    "<button class='single-number selected special' id='" +
                        "0" +
                        this_btn_number +
                        "'>" +
                        $(this).text() +
    ...
    function randomNumberGenerate() {
        ...
        let singleRandomNumberTo16 = Math.floor(
            Math.random() * (16 - 9 + 1) + 9
        );
        let singleRandomNumberTo24 = Math.floor(
            Math.random() * (24 - 17 + 1) + 17
        );
        let singleRandomNumberTo32 = Math.floor(
            Math.random() * (32 - 25 + 1) + 25
        );
        let singleRandomNumberTo40 = Math.floor(
            Math.random() * (40 - 33 + 1) + 33
        );
        let special_singleRandomNumber = Math.floor(
        ...
        $(".result-number-palate").append(
            "<button class='single-number selected' id='" +
                singleRandomNumberTo8 +
                "'>" +
                $("#" + singleRandomNumberTo8).text() +
                "</button>"
        );
        $(".result-number-palate").append(
            "<button class='single-number selected' id='" +
                singleRandomNumberTo16 +
                "'>" +
                $("#" + singleRandomNumberTo16).text() +
                "</button>"
        );
        $(".result-number-palate").append(
            "<button class='single-number selected' id='" +
                singleRandomNumberTo24 +
                "'>" +
                $("#" + singleRandomNumberTo24).text() +
                "</button>"
        );
        $(".result-number-palate").append(
            "<button class='single-number selected' id='" +
                singleRandomNumberTo32 +
                "'>" +
                $("#" + singleRandomNumberTo32).text() +
                "</button>"
        );
        $(".result-number-palate").append(
            "<button class='single-number selected' id='" +
                singleRandomNumberTo40 +
                "'>" +
                $("#" + singleRandomNumberTo40).text() +
                "</button>"
        );
        ...
        $(".number-box.special")
            ...
            .each(function () {
                ...
                $("#0" + special_singleRandomNumber).addClass(
                    "selected special"
                );
            });
        ...
        $(".result-number-palate").append(
            "<button class='single-number selected special' id='" +
                "0" +
                special_singleRandomNumber +
                "'>" +
                $("#" + special_singleRandomNumber).text() +
                "</button>"
        );
    }
    ...
    function logoChange(element) {
        var active_lottery_logo = $(element)
            .find(".lottery-icon")
            .find("img")
            .attr("src");

        $(".selected-lottery-logo")
            .find("img")
            .attr("src", active_lottery_logo);
    }

    single_lottery.each(function () {
        $(this)
            .find(".single-lottery-item")
            .on("click", function () {
                if (
                    $(this)
                        .parent()
                ...
                $(".part-lottery-info")
                    .find(".lottery-name")
                    .text(active_lottery_name);
                clearAllNumbers();
                // the below fn is commented out because it generates random number
